		...
		if($pages) {
			foreach($pages as $page) {
				...
				if( !($depth == -1 || $depth == $level)  &&
					...
					!$include)
					$children = flexipages_get_pages("child_of={$page->ID}&parent={$page->ID}&sort_column={$sort_column}&sort_order={$sort_order}&exclude={$exclude}&include={$include}&match_custom_field={$match_custom_field}&show_subpages={$show_subpages}&depth={$depth}&show_date={$show_date}&date_format={$date_format}", $level+1);
...
//		echo $args;
		...
		$page_array = flexipages_get_pages("sort_column={$sort_column}&sort_order={$sort_order}&exclude={$exclude}&include={$include}&show_subpages={$show_subpages}&hierarchy={$hierarchy}&depth={$depth}&match_custom_field={$match_custom_field}&show_home={$show_home}&child_of={$child_of}&parent={$child_of}&show_date={$show_date}&date_format={$date_format}");
...
//		echo "<pre>"; print_r ($options[$number]); echo "</pre>";
		...
		if (isset($match_custom_field) && $match_custom_field) {
		} else if ($match_custom_field_check != 'on') {
			$match_custom_field = '';
		}
			
		
		if($pagelist = flexipages("echo=0&sort_column={$sort_column}&sort_order={$sort_order}&exclude={$exclude}&include={$include}&show_subpages={$show_subpages}&hierarchy={$hierarchy}&depth={$depth}&match_custom_field={$match_custom_field}&show_home={$show_home}&show_date={$show_date}&date_format={$date_format}&dropdown={$dropdown}")){
	...
	function flexipages_widget_control($widget_args)
	{
		...
		if ( !$updated && !empty($_POST['sidebar']) ) {
			...
			foreach ( (array) $_POST['flexipages_widget'] as $widget_number => $flexipages_widget ) {
				...
				$show_custom_field_check = strip_tags(stripslashes($flexipages_widget['match_custom_field_check']));
				$match_custom_field = strip_tags(stripslashes($flexipages_widget['match_custom_field']));
				...
				$options[$widget_number] = compact('title', 'sort_column', 'sort_order', 'exinclude', 'exinclude_values', 'show_subpages_check', 'show_subpages', 'hierarchy', 'depth', 'match_custom_field_check', 'match_custom_field', 'show_home_check', 'show_home', 'show_date', 'date_format', 'dropdown');
			}
		...
		$match_custom_field_check_check = ((isset($options[$number]['match_custom_field']) && $options[$number]['match_custom_field']) || $options[$number]['match_custom_field_check'] == 'on')?' checked="checked"':'';
		$match_custom_field_display = $match_custom_field_check_check?'':' style="display:none;"';
		$match_custom_field = isset($options[$number]['match_custom_field'])?esc_attr($options[$number]['match_custom_field']):'';
		...
		<table cellpadding="10px" cellspacing="10px">
			...
			<tr>
				<td style="padding:5px 0;"><label for="flexipages-match_custom_field_check-<?php echo $number; ?>"><input type="checkbox" class="checkbox" id="flexipages-match_custom_field_check-<?php echo $number; ?>" name="flexipages_widget[<?php echo $number; ?>][match_custom_field]" onchange="if(this.checked) { getElementById('flexipages-match_custom_field-<?php echo $number; ?>').style.display='block'; } else { getElementById('flexipages-match_custom_field-<?php echo $number; ?>').style.display='none'; }"<?php echo $match_custom_field_check_check; ?> /> <?php _e('Match custom field', 'flexipages'); ?></label></td>
				<td><input<?php echo $match_custom_field_display; ?> class="widefat" type="text" name="flexipages_widget[<?php echo $number; ?>][match_custom_field]" id ="flexipages-match_custom_field-<?php echo $number; ?>" value="<?php echo htmlspecialchars($match_custom_field, ENT_QUOTES); ?>" /></td>	
			</tr>
			<tr>
